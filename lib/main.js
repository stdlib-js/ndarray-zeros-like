...
function zerosLike( x ) {
	...
	if ( !isndarrayLike( x ) ) {
		throw new TypeError( format( '0kH5t', x ) );
	}
	flg = false;
	opts = {};
	if ( arguments.length > 1 ) {
		...
		if ( !isPlainObject( options ) ) {
			throw new TypeError( format( '0kH2V', options ) );
		}
		if ( hasOwnProp( options, 'dtype' ) ) {
			dtype = options.dtype;
			if ( !isDataType( dtype ) ) {
				throw new TypeError( format( '0kH4S', 'dtype', join( DTYPES, '", "' ), dtype ) );
			}
			flg = true;
		} else {
			dtype = getDType( x );
		...
		if ( hasOwnProp( options, 'shape' ) ) {
			...
			if ( isNonNegativeInteger( sh ) ) {
				...
				throw new TypeError( format( '0kHDs', 'shape', sh ) );
			}
		} else {
			sh = getShape( x );
		}
